...
cliTools: aqua.#AquaToolSet & {
	...
	spec: tools: {
		...
		// tree-sitter CLI: required by nvim-treesitter's `main` branch to generate
		// and compile parsers from grammar (needed wherever neovim runs). The aqua
		// registry asset is a bare `.gz` single binary stored without an exec bit;
		// tomei v0.2.1 extracts bare gz (#272) and chmods placed binaries (#273),
		// so the plain aqua path now works (previously a custom curl|gunzip|chmod).
