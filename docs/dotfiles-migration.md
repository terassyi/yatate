...
| カテゴリ | ツール | インストールパターン |
|---|---|---|
| 共通 CLI ツール | rg, fd, jq, bat, delta, zellij, just, yq, gh, zoxide, gitui | aqua ToolSet (common-tools.cue) |
...
| Darwin 専用 | ~~gcloud, docker CLI~~ (tomei バグで一時除外中) | ~~aqua (darwin-tools.cue)~~ |
...
│       ├── darwin-tools.cue         # (一時除外中: docker, gcloud)
...
- [x] `dot_config/tomei/common-tools.cue` — rg, fd, jq, bat, delta, zellij, just, yq, gh, zoxide, gitui
- [x] `dot_config/tomei/lang-tools.cue` — gopls, cargo-binstall, stylua, eza, btm, tokei
- [x] `dot_config/tomei/darwin-tools.cue` — 一時除外中 (docker: tgz 未サポート, gcloud: symlink 展開失敗)
- [x] `dot_config/tomei/darwin2-extras.cue.tmpl` — ffmpeg

**既知の tomei バグ** (→ `../tomei/docs/known-issues.md`):
- tgz アーカイブ形式が未サポート (docker, skim)
- hash のみ checksum ファイル形式が未サポート (starship)
- アーカイブ内の相対シンボリンク展開失敗 (gcloud)
- delegation パターンで createSymlinks が呼ばれない (lua)
