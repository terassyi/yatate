...
- [x] ブランチ `feature/dotfiles-chezmoi-tomei` 作成
- ~~`dotfiles/.chezmoidata.toml.tmpl` 作成~~ → chezmoi の仕様上テンプレート不可。`.chezmoi.toml.tmpl` の `[data]` セクションに統合済み
- [x] `dotfiles/.chezmoi.toml.tmpl` 作成
- [x] `dotfiles/.chezmoiignore` 作成
- [x] `dotfiles/Makefile` 作成

### Phase 2: chezmoi スクリプト

- [x] `run_once_before_00-bootstrap-dirs.sh.tmpl`
- [x] `run_onchange_before_01-install-packages.sh.tmpl` (run_once → run_onchange に変更)
- [x] `run_once_before_02-install-tomei.sh.tmpl`
- [x] `run_onchange_after_01-apply-tomei.sh.tmpl`
- [x] `run_onchange_after_02-download-skim-keybindings.sh.tmpl` (追加)
- [x] `run_onchange_after_03-vscode-settings-darwin.sh.tmpl` (追加)
...
- ~~`.chezmoitemplates/fish_keybinds.fish`~~ → `config.fish.tmpl` にインライン実装済みのため不要

### Phase 4: Git・ツール設定

- [x] `dot_config/git/config.tmpl`
- [x] `dot_config/gh/config.yml`
- [x] `dot_config/starship.toml` (既存 nix 設定から変換)
- [x] `dot_config/zellij/config.kdl` (既存 nix 設定から変換)

### Phase 5: エディタ設定

- [x] Neovim: lazy.nvim ベースの `init.lua` + `lua/*.lua` に書き換え
- [x] VSCode: `settings.json`, `keybindings.json` を chezmoi に移植
- [x] Zed: `modify_settings.json.tmpl` + `keymap.json` (追加。ssh_connections 保持のため modify_ スクリプト方式)

### Phase 6: デスクトップ設定 (Linux)

- [x] `dot_config/ghostty/config`
- [x] `dot_config/dconf/gnome.ini.tmpl` (GNOME dconf 設定)
- ~~`dot_config/hypr/hyprland.conf.tmpl` + 関連設定~~ → Hyprland は使わないため移行不要
- ~~`dot_config/waybar/` (config + style)~~ → Hyprland 依存のため移行不要
- ~~`dot_config/wofi/` (config + style)~~ → Hyprland 依存のため移行不要
...
- [ ] `tomei apply ~/.config/tomei/` で全バイナリを `~/.local/bin/` や `~/.cargo/bin/` にインストール
...
- [x] `.github/workflows/dotfiles.yml` 作成
- [x] `Dockerfile` 作成（integration テスト用）
- [x] `scripts/test.sh` + `scripts/test-tools.sh` (追加)
